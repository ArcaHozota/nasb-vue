...
watch(bookId, async (id) => {
  ...
  try {
    const { data } = await api.get(`/books/${id}/chapters`);
    chapters.value = data;
...
<template>
  <div class="relative min-h-full bg-cover bg-center bg-fixed">
    ...
    <div
      class="bookaddition-card noto-serif relative overflow-hidden rounded-[18px]"
    >
      <div
      ...
      <div class="p-6">
        ...
        <div class="flex flex-wrap items-start gap-3">
          <div class="w-full md:w-[22%]">
            <label class="noto-serif mb-1 block text-sm text-gray-600"
              >書</label
            >
            <select
              v-model="bookId"
          ...
          <div class="w-full md:w-[30%]">
            <label
              class="noto-serif mb-1 flex items-center gap-1 text-sm text-gray-600"
            >
              章
              <LoaderCircle
                v-if="chapterLoading"
          ...
          <div class="w-full md:w-[22%]">
            ...
            <input
              ...
              "
              @input="
                handleVerseIdChange(($event.target as HTMLInputElement).value)
              "
            />
